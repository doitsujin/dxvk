...
namespace dxvk {
  ...
  HRESULT STDMETHODCALLTYPE D3D11Device::OpenSharedResource(
    ...
    if (!((uintptr_t)hResource & 0xc0000000)) {
    ...
    query.hGlobalShare = (uintptr_t)hResource;
    ...
    if (D3DKMTQueryResourceInfo(&query)) {
      ...
      open.hGlobalShare = (uintptr_t)hResource;
  ...
  bool D3D11Device::ConvertRuntimeDescriptor(
    ...
    if (size >= sizeof(d3dkmt.d3d11) && d3dkmt.dxgi.size == sizeof(d3dkmt.d3d11) && d3dkmt.dxgi.version == 4) {
      Logger::info(str::format("D3D11Device::ConvertRuntimeDescriptor: Found D3D11 desc with dimension: ", d3dkmt.d3d11.dimension));
      ...
      Logger::info(str::format("D3D11Device::ConvertRuntimeDescriptor: Translated D3D11 desc:"));
      Logger::info(str::format("  Width: ", desc->Width));
      Logger::info(str::format("  Height: ", desc->Height));
      Logger::info(str::format("  Depth: ", desc->Depth));
      Logger::info(str::format("  MipLevels: ", desc->MipLevels));
      Logger::info(str::format("  ArraySize: ", desc->ArraySize));
      Logger::info(str::format("  Format: ", desc->Format));
      Logger::info(str::format("  SampleDesc.Count: ", desc->SampleDesc.Count));
      Logger::info(str::format("  SampleDesc.Quality: ", desc->SampleDesc.Quality));
      Logger::info(str::format("  Usage: ", desc->Usage));
      Logger::info(str::format("  BindFlags: ", desc->BindFlags));
      Logger::info(str::format("  CPUAccessFlags: ", desc->CPUAccessFlags));
      Logger::info(str::format("  MiscFlags: ", desc->MiscFlags));
      Logger::info(str::format("  TextureLayout: ", desc->TextureLayout));
      return true;
    }

    if (size >= sizeof(d3dkmt.d3d9) && d3dkmt.dxgi.size == sizeof(d3dkmt.d3d9) && d3dkmt.dxgi.version == 1) {
      Logger::info(str::format("D3D11Device::ConvertRuntimeDescriptor: Found D3D9 desc: ", d3dkmt.d3d9.type));
      Logger::info(str::format("  dxgi.width: ", d3dkmt.d3d9.dxgi.width));
      Logger::info(str::format("  dxgi.height: ", d3dkmt.d3d9.dxgi.height));
      Logger::info(str::format("  format: ", d3dkmt.d3d9.format));
      Logger::info(str::format("  usage: ", d3dkmt.d3d9.usage));
      if (d3dkmt.d3d9.type == D3DRTYPE_TEXTURE) {
        Logger::info(str::format("  texture.width: ", d3dkmt.d3d9.texture.width));
        Logger::info(str::format("  texture.height: ", d3dkmt.d3d9.texture.height));
        Logger::info(str::format("  texture.depth: ", d3dkmt.d3d9.texture.depth));
        Logger::info(str::format("  texture.levels: ", d3dkmt.d3d9.texture.levels));
      } else if (d3dkmt.d3d9.type == D3DRTYPE_SURFACE) {
        Logger::info(str::format("  surface.width: ", d3dkmt.d3d9.surface.width));
        Logger::info(str::format("  surface.height: ", d3dkmt.d3d9.surface.height));
      } else {
      ...
      Logger::info(str::format("D3D11Device::ConvertRuntimeDescriptor: Translated D3D9 desc:"));
      Logger::info(str::format("  Width: ", desc->Width));
      Logger::info(str::format("  Height: ", desc->Height));
      Logger::info(str::format("  Depth: ", desc->Depth));
      Logger::info(str::format("  MipLevels: ", desc->MipLevels));
