...
namespace dxvk {
  ...
  HRESULT D3D11SwapChain::PresentImage(UINT SyncInterval) {
    ...
    for (uint32_t i = 0; i < SyncInterval || i < 1; i++) {
      if (!m_presenter->recordPresentCommands(m_swapImageView, VkRect2D(), VkRect2D(),
          i + 1 >= SyncInterval ? m_frameLatencySignal : nullptr, frameId, i))
        return DXGI_STATUS_OCCLUDED;

      SubmitPresent(immediateContext, i);
    }
  ...
  void D3D11SwapChain::CreateBackBuffer() {
    ...
    m_parent->InitTexture(GetCommonTexture(m_backBuffer));
  }
