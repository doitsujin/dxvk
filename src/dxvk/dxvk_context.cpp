...
namespace dxvk {
  ...
  void DxvkContext::changeImageLayout(
    ...
    if (image->info().layout != layout) {
      ...
      // Only emit one transition to the destination layout
      DxvkResourceBatch accessBatch;
      accessBatch.add(*image, image->getAvailableSubresources(),
        layout, image->info().stages, image->info().access, VK_FALSE);
  ...
  void DxvkContext::clearBuffer(
    const Rc<DxvkBuffer>&       buffer,
          ...
          uint32_t              value) {
    DxvkResourceBatch accessBatch;
    accessBatch.add(*buffer, offset, length,
  ...
  void DxvkContext::clearBufferView(
    ...
    DxvkResourceBatch accessBatch;
    accessBatch.add(*bufferView, offset, length,
      VK_PIPELINE_STAGE_2_TRANSFER_BIT, VK_ACCESS_2_TRANSFER_WRITE_BIT);
  ...
  void DxvkContext::copyPackedBufferImage(
    ...
    if (srcBuffer == dstBuffer
     ...
     && srcView->info().offset + srcView->info().size > dstView->info().offset) {
      ...
      Rc<DxvkBufferView> tmpView = tmpBuffer->createView(viewInfo);

      DxvkResourceBatch accessBatch;
      accessBatch.add(*tmpView, VK_PIPELINE_STAGE_2_TRANSFER_BIT, VK_ACCESS_2_TRANSFER_WRITE_BIT);
      accessBatch.add(*srcView, VK_PIPELINE_STAGE_2_TRANSFER_BIT, VK_ACCESS_2_TRANSFER_READ_BIT);
    ...
    DxvkResourceBatch accessBatch;
    accessBatch.add(*dstView, VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT, VK_ACCESS_2_SHADER_WRITE_BIT);
    accessBatch.add(*srcView, VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT, VK_ACCESS_2_SHADER_READ_BIT);
    syncResources(DxvkCmdBuffer::ExecBuffer, accessBatch, false);
  ...
  void DxvkContext::launchCuKernelNVX(
    ...
    DxvkResourceBatch batch;

    for (auto& r : buffers) {
      VkPipelineStageFlags2 access = (r.second.test(DxvkAccess::Read) ? VK_ACCESS_2_MEMORY_READ_BIT : 0u)
                                   | (r.second.test(DxvkAccess::Write) ? VK_ACCESS_2_MEMORY_WRITE_BIT : 0u);
      batch.add(*r.first, 0u, r.first->info().size, VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT, access);
    }
    ...
    for (auto& r : images) {
      ...
      batch.add(*r.first, r.first->getAvailableSubresources(),
        r.first->info().layout, VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT, access, false);
    }

    syncResources(DxvkCmdBuffer::ExecBuffer, batch, true);
  ...
  void DxvkContext::blitImageFb(
    ...
    dstView = ensureImageViewCompatibility(dstView, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT);
    ...
    DxvkResourceBatch accessBatch;
    accessBatch.add(*dstView, dstView->getLayout(),
      VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT,
      VK_ACCESS_2_COLOR_ATTACHMENT_WRITE_BIT | VK_ACCESS_2_COLOR_ATTACHMENT_READ_BIT, false);
    accessBatch.add(*srcView, srcView->getLayout(),
      VK_PIPELINE_STAGE_2_FRAGMENT_SHADER_BIT,
      VK_ACCESS_2_SHADER_READ_BIT, false);
  ...
  void DxvkContext::blitImageHw(
    ...
    DxvkResourceBatch accessBatch;
    accessBatch.add(*dstView, dstLayout, VK_PIPELINE_STAGE_2_TRANSFER_BIT, VK_ACCESS_2_TRANSFER_WRITE_BIT, false);
    accessBatch.add(*srcView, srcLayout, VK_PIPELINE_STAGE_2_TRANSFER_BIT, VK_ACCESS_2_TRANSFER_READ_BIT, false);
    syncResources(DxvkCmdBuffer::ExecBuffer, accessBatch, true);
  ...
  void DxvkContext::copyBufferToImageHw(
    ...
    VkImageLayout dstImageLayout = image->pickLayout(VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);

    DxvkResourceBatch accessBatch;
    accessBatch.add(*buffer, bufferOffset, dataSize, VK_PIPELINE_STAGE_2_TRANSFER_BIT, VK_ACCESS_2_TRANSFER_READ_BIT);

    auto& imageAccess = accessBatch.add(*image, vk::makeSubresourceRange(dstSubresource),
      dstImageLayout, VK_PIPELINE_STAGE_2_TRANSFER_BIT, VK_ACCESS_2_TRANSFER_WRITE_BIT,
    ...
    syncResources(cmdBuffer, accessBatch, true);

    auto bufferSlice = buffer->getSliceInfo(bufferOffset, dataSize);

    copyImageBufferData<true>(cmdBuffer,
      image, imageSubresource, imageOffset, imageExtent, dstImageLayout,
  ...
  void DxvkContext::copyBufferToImageFb(
    ...
    auto& imageAccess = accessBatch.add(*imageView, imageView->getLayout(), stages, access, discard);
    imageAccess.imageOffset = imageOffset;
    imageAccess.imageExtent = imageExtent;

    accessBatch.add(*bufferView, VK_PIPELINE_STAGE_2_FRAGMENT_SHADER_BIT, VK_ACCESS_2_SHADER_READ_BIT);
    syncResources(DxvkCmdBuffer::ExecBuffer, accessBatch, true);
  ...
  void DxvkContext::clearImageViewFb(
    ...
    if (attachmentIndex < 0) {
      ...
      VkExtent3D renderExtent = imageView->mipLevelExtent(0);
      ...
      VkRenderingInfo renderingInfo = { VK_STRUCTURE_TYPE_RENDERING_INFO };
      renderingInfo.renderArea.extent = { renderExtent.width, renderExtent.height };
      renderingInfo.layerCount = imageView->info().layerCount;
      ...
      VkImageSubresourceRange clearSubresource = imageView->imageSubresources();
      clearSubresource.aspectMask = aspect;

      DxvkResourceBatch accessBatch;
      accessBatch.add(*imageView, clearLayout, clearStages, clearAccess,
        imageView->image()->isFullSubresource(vk::pickSubresourceLayers(clearSubresource, 0u), extent));
      syncResources(DxvkCmdBuffer::ExecBuffer, accessBatch, true);
  ...
  void DxvkContext::clearImageViewCs(
    ...
    auto& imageAccess = accessBatch.add(*imageView, imageView->getLayout(),
      VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT, VK_ACCESS_2_SHADER_WRITE_BIT, VK_FALSE);
    imageAccess.imageOffset = offset;
    imageAccess.imageExtent = extent;
  ...
  void DxvkContext::copySparseBufferPages(
    ...
    DxvkResourceBatch accessBatch;
    accessBatch.add(*buffer, offset, SparseMemoryPageSize * pageCount,
      VK_PIPELINE_STAGE_2_TRANSFER_BIT,
      ToBuffer ? VK_ACCESS_2_TRANSFER_WRITE_BIT : VK_ACCESS_2_TRANSFER_READ_BIT);
    accessBatch.add(*sparse, 0u, sparse->info().size,
      VK_PIPELINE_STAGE_2_TRANSFER_BIT,
  ...
  void DxvkContext::copySparseImagePages(
    ...
    accessBatch.add(*buffer, offset, SparseMemoryPageSize * pageCount,
      VK_PIPELINE_STAGE_2_TRANSFER_BIT,
      ToBuffer ? VK_ACCESS_2_TRANSFER_WRITE_BIT : VK_ACCESS_2_TRANSFER_READ_BIT);
    accessBatch.add(*sparse, sparse->getAvailableSubresources(),
      transferLayout, VK_PIPELINE_STAGE_2_TRANSFER_BIT, transferAccess, VK_FALSE);
    syncResources(DxvkCmdBuffer::ExecBuffer, accessBatch, true);
