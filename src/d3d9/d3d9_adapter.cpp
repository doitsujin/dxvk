...
namespace dxvk {
  ...
  HRESULT D3D9Adapter::CheckDeviceFormat(
    ...
    // in D3D8 by modern AMD drivers)
    if (rt && CheckFormat == D3D9Format::RESZ && surface)
      return ( m_vendorId == uint32_t(DxvkGpuVendor::Amd)
    ...
    if (CheckFormat == D3D9Format::SSAA && surface) {
      if (!isD3D8Compatible)
    ...
    if (CheckFormat == D3D9Format::NVDB && surface)
      return (!isD3D8Compatible &&
              m_adapter->features().core.features.depthBounds &&
              m_vendorId == uint32_t(DxvkGpuVendor::Nvidia))
        ? D3D_OK
        : D3DERR_NOTAVAILABLE;
    ...
    if (CheckFormat == D3D9Format::R2VB && surface) {
      if (!isD3D8Compatible)
    ...
    if (CheckFormat == D3D9Format::INST && surface)
    return (!isD3D8Compatible &&
