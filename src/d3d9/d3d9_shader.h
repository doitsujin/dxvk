...
namespace dxvk {
  ...
  public:
    ...
    inline VkImageViewType GetImageViewType(uint32_t samplerSlot) const {
      const uint32_t offset = samplerSlot * 2;
      const uint32_t mask = 0b11;
      return static_cast<VkImageViewType>(((m_textureTypes >> offset) & mask) + VK_IMAGE_VIEW_TYPE_1D);
    }
  ...
  template<typename T>
  inline const D3D9CommonShader* GetCommonShader(const T& pShader) {
