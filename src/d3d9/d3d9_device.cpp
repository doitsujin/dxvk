...
namespace dxvk {
  ...
  HRESULT STDMETHODCALLTYPE D3D9DeviceEx::Reset(D3DPRESENT_PARAMETERS* pPresentationParameters) {
    ...
    */
    if (unlikely(m_losableResourceCounter.load() != 0 && !isExtended &&
                !m_d3dCompatibility.test(D3DCompatibility::D3D7) && m_d3d9Options.countLosableResources)) {
  ...
  HRESULT STDMETHODCALLTYPE D3D9DeviceEx::CreateOffscreenPlainSurfaceEx(
    ...
    if (unlikely(!m_d3dCompatibility.test(D3DCompatibility::D3D7) &&
  ...
  HRESULT STDMETHODCALLTYPE D3D9DeviceEx::CreateDepthStencilSurfaceEx(
    ...
    desc.IsBackBuffer       = FALSE;
    desc.IsAttachmentOnly   = TRUE;
    desc.IsLockable         = !m_d3dCompatibility.test(D3DCompatibility::D3D7) ? IsLockableDepthStencilFormat(desc.Format) : TRUE;
